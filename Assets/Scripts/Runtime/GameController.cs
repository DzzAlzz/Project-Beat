...
namespace ProjectBeat.Runtime
{
    public class GameController : MonoBehaviour
    {
        ...
        public float       CalibratedSongPosition => conductor == null ? 0f : conductor.GetLiveSongPosition() + timingOffsetSeconds;
        ...
        private void Start()
        {
            ...
            if (Beatmap != null)
            {
                ...
                if (ProfileAchievementStorage.TryUnlockById("FIRST_SONG", out var firstSongAchievement))
                    AchievementNotification.Show(firstSongAchievement);
                TutorialOverlayController.EnsureForCurrentLevel();
                UnifiedLoadingScreen.Hide(0.25f);
            }
        }
        ...
        private void OnGUI()
        {
            // Avance 84: la pantalla CARGANDO... se unifica en UnifiedLoadingScreen.
            // Se evita dibujar texto amarillo/transparente con OnGUI para que no haya estilos distintos.
            if (showResultsExitLoading) return;
        ...
        private void ApplyResponsivenessSettings()
        {
            ...
            ProjectBeatPerformanceSettings.ApplyHighRefreshMode();
            Application.targetFrameRate = Mathf.Max(ProjectBeatPerformanceSettings.TargetFrameRate, targetFrameRate);
        ...
        public void FinishSong()
        {
            ...
            if (LevelManager.Instance != null && LevelManager.Instance.Levels != null)
            {
                ...
                totalLevels = LevelManager.Instance.Levels.Length;
                ProfileStatsStorage.MarkLevelCompleted(currentLevelIndex, totalLevels, firstArcade);
            }

            var unlockedAchievements = ProfileAchievementStorage.EvaluateAfterLevelComplete(scoreManager, currentLevelIndex, totalLevels, levelName);
            for (int i = 0; i < unlockedAchievements.Count; i++)
                AchievementNotification.Show(unlockedAchievements[i]);
