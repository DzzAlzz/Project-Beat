...
namespace ProjectBeat.Runtime
{
    ...
    public static class ProfileStatsStorage
    {
        ...
        {
            return TryGetCurrentProfile(out profileId, out profileName, out _);
        }

        public static bool TryGetCurrentProfile(out string profileId, out string profileName, out bool partidaCreada)
        {
        ...
        public static bool HasLoadedProfileGame()
        {
            return TryGetCurrentProfile(out _, out _, out bool partidaCreada) && partidaCreada;
        }

        public static void ResetProgressForCurrentProfile(int firstArcadeLevelIndex)
        {
            if (!TryGetCurrentProfile(out string profileId, out string profileName, out bool partidaCreada)) return;
            ProfileStatsData data = LoadStats(profileId, profileName);
            data.levels = new List<LevelStats>();
            data.unlockedLevelIndex = Mathf.Max(0, firstArcadeLevelIndex);
            SaveStats(data);
        }

        public static int GetUnlockedLevelIndex(int firstArcadeLevelIndex)
        {
            if (!TryGetCurrentProfile(out string profileId, out string profileName, out bool partidaCreada) || !partidaCreada)
                return int.MaxValue;

            ProfileStatsData data = LoadStats(profileId, profileName);
            if (data.unlockedLevelIndex < firstArcadeLevelIndex)
            {
                data.unlockedLevelIndex = Mathf.Max(0, firstArcadeLevelIndex);
                SaveStats(data);
            }
            return data.unlockedLevelIndex;
        }

        public static bool IsLevelUnlocked(int levelIndex, int firstArcadeLevelIndex)
        {
            if (!TryGetCurrentProfile(out _, out _, out bool partidaCreada) || !partidaCreada)
                return true;

            int unlocked = GetUnlockedLevelIndex(firstArcadeLevelIndex);
            return levelIndex <= unlocked;
        }

        public static void MarkLevelCompleted(int levelIndex, int totalLevels, int firstArcadeLevelIndex)
        {
            if (!TryGetCurrentProfile(out string profileId, out string profileName, out bool partidaCreada) || !partidaCreada) return;
            ...
            if (levelIndex >= data.unlockedLevelIndex && levelIndex + 1 < totalLevels)
                data.unlockedLevelIndex = levelIndex + 1;
        ...
        // Avance 83: punto central de autoguardado. PlayerPrefs normalmente
        // escribe al cerrar, pero aqui forzamos el flush para evitar perder
        // perfiles/progreso si el jugador sale del ejecutable rapidamente.
