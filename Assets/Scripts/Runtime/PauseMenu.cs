...
namespace ProjectBeat.Runtime
{
    ...
    public class PauseMenu : MonoBehaviour
    {
        ...
        private void RefreshLevelSelectLabels()
        {
            ...
            {
                levelNameText.alignment = TextAlignmentOptions.Center;
                levelNameText.fontSize = 58f;
                levelNameText.characterSpacing = 4.5f;
                levelNameText.text =
                    "<size=19><color=#FFD6A2>SELECCIONA TU TRACK</color></size>\n" +
                    "<size=42><color=#00F1FF><</color></size>  " +
                    "<b><color=#FFB23C>" + level.levelName + "</color></b>" +
                    "  <size=42><color=#00F1FF>></color></size>\n" +
                    "<size=23><color=#FFE3B8>PISTA " + (idx + 1) + " DE " + total + "</color></size>";
            }
            ...
            if (levelArtistText != null)
            {
                ...
                levelArtistText.characterSpacing = 1.7f;
                levelArtistText.text = "<color=#FF7A18>Nivel " + (idx + 1) + "</color>  <color=#00F1FF>•</color>  <color=#FFF5E0>LISTO PARA JUGAR</color>";
            }
            ...
            if (levelHintText != null)
            {
                ...
                levelHintText.text = "<color=#00F1FF>[A/D]</color> Cambiar pista    <color=#FFB23C>[ENTER]</color> Iniciar    <color=#FF7A18>[ESC]</color> Volver    <color=#FFE3B8>[MOUSE]</color> Click";
            }
        }
        ...
        private void BuildCommercialLevelSelect()
        {
            ...
            // Avance 59: el selector debe ser una pantalla completa propia.
            // Esto evita que se vean carriles, HUD o elementos del gameplay detras.
            ...
            Color metalDark = new Color(0.18f, 0.070f, 0.012f, 1f);
            Color metalBright = new Color(1f, 0.48f, 0.055f, 0.34f);
            Color warmGold = new Color(1f, 0.70f, 0.20f, 0.46f);
            Color cyanSoft = new Color(0f, 0.92f, 1f, 0.30f);

            CreateFullScreenImage(levelSelectGroup.transform, "PB_UI_LevelMetalOpaqueBackground", metalDark, 0);
            CreateFullScreenImage(levelSelectGroup.transform, "PB_UI_LevelWarmMetalWash", new Color(0.95f, 0.25f, 0.035f, 0.58f), 1);
            CreateFullScreenImage(levelSelectGroup.transform, "PB_UI_LevelGoldenVignette", new Color(0.40f, 0.14f, 0.00f, 0.38f), 2);

            levelSelectorAura = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelOrangeMetalAura", new Vector2(0f, 10f), new Vector2(1180f, 620f), metalBright, 3);
            CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelGoldenGlowTop", new Vector2(0f, 250f), new Vector2(920f, 150f), warmGold, 4);
            CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelCyanSideGlow", new Vector2(-500f, -165f), new Vector2(440f, 95f), cyanSoft, 5);
            CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelCyanSideGlow_R", new Vector2(500f, -165f), new Vector2(440f, 95f), cyanSoft, 6);

            levelSelectorLines = new Image[12];
            for (int i = 0; i < levelSelectorLines.Length; i++)
            {
                ...
                Vector2 pos = new Vector2(side * (360f + local * 76f), 230f - local * 82f);
                Vector2 size = new Vector2(190f + local * 26f, 4f);
                Color color = local % 2 == 0 ? new Color(1f, 0.46f, 0.06f, 0.46f) : new Color(0f, 0.92f, 1f, 0.30f);
                levelSelectorLines[i] = CreateLevelDecorLine(levelSelectGroup.transform, "PB_UI_LevelMetalLine_" + i, pos, size, color, 7 + i, side * -10f);
            }

            levelSelectorRings = new Image[4];
            levelSelectorRings[0] = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelRing_L", new Vector2(-425f, 80f), new Vector2(150f, 150f), new Color(1f, 0.34f, 0.02f, 0.18f), 20);
            levelSelectorRings[1] = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelRing_R", new Vector2(425f, 80f), new Vector2(150f, 150f), new Color(1f, 0.34f, 0.02f, 0.18f), 21);
            levelSelectorRings[2] = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelRing_Cyan_L", new Vector2(-520f, -185f), new Vector2(95f, 95f), new Color(0f, 0.92f, 1f, 0.16f), 22);
            levelSelectorRings[3] = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelRing_Cyan_R", new Vector2(520f, -185f), new Vector2(95f, 95f), new Color(0f, 0.92f, 1f, 0.16f), 23);

            levelSelectorBars = new Image[22];
            for (int i = 0; i < levelSelectorBars.Length; i++)
            {
                float x = -610f + i * 58f;
                float h = 34f + (i % 5) * 18f;
                Color barColor = i % 3 == 0 ? new Color(0f, 0.92f, 1f, 0.35f) : new Color(1f, 0.42f, 0.035f, 0.42f);
                levelSelectorBars[i] = CreateLevelBar(levelSelectGroup.transform, "PB_UI_LevelEqualizerBar_" + i, new Vector2(x, -315f), new Vector2(18f, h), barColor, 26 + i);
            }

            levelSelectorParticles = new Image[24];
            for (int i = 0; i < levelSelectorParticles.Length; i++)
            {
                float x = -590f + (i * 83f) % 1180f;
                float y = -250f + (i * 47f) % 500f;
                float size = 5f + (i % 4) * 2f;
                Color particleColor = i % 2 == 0 ? new Color(1f, 0.58f, 0.05f, 0.55f) : new Color(0f, 0.92f, 1f, 0.38f);
                levelSelectorParticles[i] = CreateLevelBar(levelSelectGroup.transform, "PB_UI_LevelSpark_" + i, new Vector2(x, y), new Vector2(size, size), particleColor, 50 + i);
            }

            levelSelectorCardGlow = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelCardOuterGlow", Vector2.zero, new Vector2(870f, 520f), new Color(1f, 0.38f, 0.03f, 0.28f), 75);
            RectTransform card = CreateCard(levelSelectGroup.transform, "PB_UI_LevelCard", new Vector2(820f, 460f), Vector2.zero, 76);
            if (card != null)
            {
                Image cardImage = card.GetComponent<Image>();
                if (cardImage != null)
                    cardImage.color = new Color(0.16f, 0.060f, 0.015f, 0.94f);

                Outline outline = card.GetComponent<Outline>();
                if (outline != null)
                {
                    outline.effectColor = new Color(1f, 0.50f, 0.08f, 0.55f);
                    outline.effectDistance = new Vector2(2.4f, -2.4f);
                }
            }

            CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelNameBackGlow", new Vector2(0f, 42f), new Vector2(710f, 122f), new Color(1f, 0.35f, 0.02f, 0.18f), 77);
            levelSelectorTitleGlow = CreateFloatingGlow(levelSelectGroup.transform, "PB_UI_LevelTitleGlow", new Vector2(0f, 158f), new Vector2(500f, 52f), new Color(0f, 0.92f, 1f, 0.18f), 78);

            CreateLine(levelSelectGroup.transform, "PB_UI_LevelTopNeon", new Vector2(0f, 214f), new Vector2(690f, 5f), NeonOrange, 79);
            CreateLine(levelSelectGroup.transform, "PB_UI_LevelTopCyanAccent", new Vector2(0f, 196f), new Vector2(560f, 3f), NeonCyan, 80);
            CreateLine(levelSelectGroup.transform, "PB_UI_LevelBottomNeon", new Vector2(0f, -214f), new Vector2(470f, 4f), NeonCyan, 81);
            CreateLine(levelSelectGroup.transform, "PB_UI_LevelBottomOrangeAccent", new Vector2(0f, -190f), new Vector2(640f, 3f), NeonOrange, 82);
            CreateTmp(levelSelectGroup.transform, "PB_UI_LevelBadge", "ARCADE SELECT", new Vector2(0f, 156f), new Vector2(520f, 32f), 19f, NeonCyan, 83, FontStyles.Bold, 4f);
            ...
            if (levelNameText != null)
            {
                ...
                rt.sizeDelta = new Vector2(760f, 174f);
                levelNameText.transform.SetSiblingIndex(84);
            }
            ...
            if (levelArtistText != null)
            {
                ...
                rt.sizeDelta = new Vector2(690f, 48f);
                levelArtistText.transform.SetSiblingIndex(85);
            }
            ...
            if (levelHintText != null)
            {
                ...
                rt.sizeDelta = new Vector2(760f, 38f);
                levelHintText.transform.SetSiblingIndex(86);
            }

            // Avance 47: flechas, iniciar y volver clickeables en el selector.
            CreateLevelMouseButton(levelSelectGroup.transform, "PB_UI_LevelMouse_Left", new Vector2(-318f, 44f), new Vector2(125f, 150f), () => ChangeLevelWithMouse(-1));
            CreateLevelMouseButton(levelSelectGroup.transform, "PB_UI_LevelMouse_Right", new Vector2(318f, 44f), new Vector2(125f, 150f), () => ChangeLevelWithMouse(1));
            CreateLevelMouseButton(levelSelectGroup.transform, "PB_UI_LevelMouse_Start", new Vector2(0f, -72f), new Vector2(690f, 76f), ConfirmLevelSelect);
            TMP_Text backText = CreateTmp(levelSelectGroup.transform, "PB_UI_LevelBackButton", "<color=#FFB23C><b>VOLVER</b></color>", new Vector2(0f, -198f), new Vector2(260f, 34f), 20f, NeonOrange, 87, FontStyles.Bold, 1.0f);
            CreateLevelMouseButton(levelSelectGroup.transform, "PB_UI_LevelMouse_Back", new Vector2(0f, -198f), new Vector2(280f, 46f), ExitLevelSelect);
        ...
        private void AnimateLevelSelectVisuals()
        {
            ...
            {
                levelSelectorAura.color = Color.Lerp(new Color(1f, 0.24f, 0.02f, 0.20f), new Color(1f, 0.62f, 0.08f, 0.36f), glow);
            ...
            {
                levelSelectorCardGlow.color = Color.Lerp(new Color(1f, 0.30f, 0.02f, 0.20f), new Color(1f, 0.62f, 0.10f, 0.36f), glow);
            ...
            {
                levelSelectorTitleGlow.color = Color.Lerp(new Color(0f, 0.92f, 1f, 0.12f), new Color(1f, 0.48f, 0.04f, 0.28f), glow);
            }
            ...
            if (levelSelectorBars != null)
            {
                for (int i = 0; i < levelSelectorBars.Length; i++)
                {
                    ...
                    float beat = 0.5f + 0.5f * Mathf.Sin(t * 1.2f + i * 0.62f);
                    rt.sizeDelta = new Vector2(rt.sizeDelta.x, 28f + beat * (32f + (i % 4) * 12f));
                    Color target = i % 3 == 0 ? new Color(0f, 0.92f, 1f, 0.24f + beat * 0.22f) : new Color(1f, 0.42f, 0.035f, 0.28f + beat * 0.28f);
                    bar.color = target;
                }
            }
            ...
            if (levelSelectorLines != null)
            {
                for (int i = 0; i < levelSelectorLines.Length; i++)
                {
                    ...
                    p.x += Mathf.Sin(t * 0.28f + i) * Time.unscaledDeltaTime * 18f;
                    if (p.x > 720f) p.x = -720f;
                    if (p.x < -720f) p.x = 720f;
                    rt.anchoredPosition = p;
                    float alpha = 0.22f + (0.5f + 0.5f * Mathf.Sin(t * 0.9f + i)) * 0.30f;
                    Color c = line.color;
                    c.a = alpha;
                    line.color = c;
                }
            ...
            if (levelSelectorParticles != null)
            {
                for (int i = 0; i < levelSelectorParticles.Length; i++)
                {
                    ...
                    if (pos.y > 290f) pos.y = -280f;
                    rt.anchoredPosition = pos;
                    Color c = particle.color;
                    c.a = 0.18f + (0.5f + 0.5f * Mathf.Sin(t * 1.1f + i * 0.55f)) * 0.42f;
                    particle.color = c;
                }
            }
            ...
            if (levelSelectorRings != null)
            {
                for (int i = 0; i < levelSelectorRings.Length; i++)
                {
                    ...
                    Color c = ring.color;
                    c.a = 0.10f + (0.5f + 0.5f * Mathf.Sin(t * 0.9f + i)) * 0.13f;
                    ring.color = c;
                }
            }
        }
