...
namespace ProjectBeat.Runtime
{
    ...
    public class AchievementNotification : MonoBehaviour
    {
        ...
        private struct PendingAchievement
        {
            public ProfileAchievementStorage.AchievementDefinition achievement;
            public bool playSound;
        }

        private readonly Queue<PendingAchievement> pending = new Queue<PendingAchievement>();
        ...
        private static readonly Color NeonGold = new Color(0.0f, 0.94f, 1f, 1f);
        private const string UnlockClipResourcePath = "Audio/UI/AchievementUnlock";
        ...
        public static void Show(ProfileAchievementStorage.AchievementDefinition achievement, bool playSound)
        {
            if (achievement == null) return;
            EnsureInstance();
            instance.ShowInternal(achievement, playSound);
        }
        ...
        private void BuildUI()
        {
            ...
            iconImage = CreateImage("AchievementIcon", panel, NeonGold);
            iconImage.sprite = AchievementIconFactory.MakeIcon("MASTER_RHYTHM", 128);
            iconImage.rectTransform.sizeDelta = new Vector2(58f, 58f);
            iconImage.rectTransform.anchoredPosition = new Vector2(-168f, 0f);
        ...
        private void ShowInternal(ProfileAchievementStorage.AchievementDefinition achievement, bool playSound)
        {
            pending.Enqueue(new PendingAchievement { achievement = achievement, playSound = playSound });
            if (routine == null)
        ...
        private IEnumerator ShowRoutine()
        {
            ...
            {
                PendingAchievement item = pending.Dequeue();
                ProfileAchievementStorage.AchievementDefinition achievement = item.achievement;
                if (achievement == null) continue;
                if (nameText != null) nameText.text = achievement.title;
                if (descText != null) descText.text = achievement.description;
                if (iconImage != null) iconImage.sprite = AchievementIconFactory.MakeIcon(achievement.id, 128);
        ...
        private void BuildAudio()
        {
            ...
            unlockAudioSource.spatialBlend = 0f;
            unlockAudioSource.ignoreListenerPause = true;
            unlockAudioSource.priority = 32;
            unlockAudioSource.volume = 0.72f;

            unlockClip = Resources.Load<AudioClip>(UnlockClipResourcePath);
            if (unlockClip == null)
                unlockClip = GenerateFallbackUnlockClip();
        }
        ...
        private void PlayUnlockSound()
        {
            ...
            if (unlockAudioSource == null || unlockClip == null)
                return;

            if (unlockAudioSource.isPlaying)
                unlockAudioSource.Stop();

            unlockAudioSource.PlayOneShot(unlockClip, 0.72f);
        }

        private AudioClip GenerateFallbackUnlockClip()
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                float env;
                if (t < 0.025f) env = t / 0.025f;
                else env = Mathf.Exp(-4.6f * (t - 0.025f));

                float toneA = Mathf.Sin(2f * Mathf.PI * 880f * t);
                float toneB = Mathf.Sin(2f * Mathf.PI * 1320f * Mathf.Max(0f, t - 0.055f));
                float toneC = Mathf.Sin(2f * Mathf.PI * 1760f * Mathf.Max(0f, t - 0.120f));
                float shimmer = Mathf.Sin(2f * Mathf.PI * 15f * t) * 0.10f + 0.90f;
                data[i] = Mathf.Clamp((toneA * 0.34f + toneB * 0.24f + toneC * 0.16f) * env * shimmer, -0.85f, 0.85f);
            }

            AudioClip clip = AudioClip.Create("PB_AchievementUnlock_Runtime", sampleCount, 1, sampleRate, false);
            clip.SetData(data, 0);
            return clip;
        }
