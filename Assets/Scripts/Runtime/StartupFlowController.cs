...
namespace ProjectBeat.Runtime
{
    ...
    public class StartupFlowController : MonoBehaviour
    {
        ...
        private void CreateMainMenuAchievementsButton(Transform parent)
        {
            ...
            achievementsButtonGlow = CreateImage("AchievementsButtonGlow", go.transform, new Color(1f, 0.78f, 0.16f, 0.18f));
            achievementsButtonGlow.sprite = MakeRadialGlowSprite(96);
            achievementsButtonGlow.rectTransform.sizeDelta = new Vector2(122f, 122f);
            ...
            achievementsButtonIconImage.sprite = MakeTrophySprite(96);
            achievementsButtonIconImage.color = new Color(1.0f, 0.82f, 0.20f, 1f);
            ...
            enter.callback.AddListener((_) =>
            {
                ...
                if (achievementsButtonBg != null) achievementsButtonBg.color = new Color(0.24f, 0.06f, 0.34f, 0.98f);
            ...
            exit.callback.AddListener((_) =>
            {
                ...
                if (achievementsButtonIconImage != null) achievementsButtonIconImage.color = new Color(1.0f, 0.82f, 0.20f, 1f);
            });
            trigger.triggers.Add(exit);
        ...
        private void BuildAchievementsPanel(Transform parent)
        {
            ...
            Image dim = CreateImage("AchievementsDim", root.transform, new Color(0.010f, 0.000f, 0.040f, 0.74f));
            Stretch(dim.rectTransform);
            dim.raycastTarget = true;

            Image glow = CreateImage("AchievementsGlow", root.transform, new Color(1f, 0.22f, 0.72f, 0.18f));
            glow.sprite = MakeRadialGlowSprite(160);
            glow.rectTransform.sizeDelta = new Vector2(880f, 620f);
            glow.rectTransform.anchoredPosition = Vector2.zero;
            glow.raycastTarget = false;

            Image box = CreateImage("AchievementsPanelBox", root.transform, new Color(0.095f, 0.030f, 0.160f, 0.98f));
            box.sprite = MakeSprite(Color.white);
            box.type = Image.Type.Sliced;
            box.rectTransform.sizeDelta = new Vector2(760f, 580f);
            box.rectTransform.anchoredPosition = Vector2.zero;
            box.raycastTarget = true;
            achievementsPanelBox = box.rectTransform;

            CreateLine("AchievementsTopLine", box.transform, new Vector2(0f, 258f), new Vector2(600f, 4f), NeonCyan).raycastTarget = false;
            CreateLine("AchievementsBottomLine", box.transform, new Vector2(0f, -258f), new Vector2(600f, 4f), NeonPink).raycastTarget = false;
            ...
            Image bodyBox = CreateImage("AchievementsBodyBox", box.transform, new Color(0.050f, 0.012f, 0.105f, 0.78f));
            bodyBox.sprite = MakeSprite(Color.white);
            bodyBox.type = Image.Type.Sliced;
            bodyBox.rectTransform.sizeDelta = new Vector2(660f, 386f);
            bodyBox.rectTransform.anchoredPosition = new Vector2(0f, -28f);
            bodyBox.raycastTarget = false;

            achievementsPanelBodyText = CreateText("AchievementsBody", bodyBox.transform, "", 18, TextNormal, FontStyles.Bold);
            achievementsPanelBodyText.alignment = TextAlignmentOptions.TopLeft;
            achievementsPanelBodyText.enableWordWrapping = true;
            achievementsPanelBodyText.richText = true;
            achievementsPanelBodyText.rectTransform.sizeDelta = new Vector2(610f, 350f);
            achievementsPanelBodyText.rectTransform.anchoredPosition = new Vector2(0f, -4f);

            TMP_Text closeText;
            achievementsCloseBg = CreateHelpButton(box.transform, "AchievementsClose", "X", new Vector2(336f, 226f), false, out closeText);
        ...
        private void RefreshAchievementsPanel()
        {
            if (!ProfileStatsStorage.TryGetCurrentProfile(out string profileId, out string profileName))
            {
                ...
                if (achievementsPanelBodyText != null)
                    achievementsPanelBodyText.text = "<align=\"center\"><color=#FF66D9><b>NO HAY PERFIL CARGADO</b></color>\n\nCarga una partida desde <color=#00F1FF>PERFIL</color> para ver tus logros.</align>";
                return;
            }

            if (achievementsPanelProfileText != null)
                achievementsPanelProfileText.text = "PERFIL ACTUAL: " + profileName;
            if (achievementsPanelBodyText == null) return;
            ...
            for (int i = 0; i < defs.Length; i++)
            {
                ...
                string icon = isUnlocked ? "<color=#FFE86B>★</color>" : "<color=#6B5A8E>■</color>";
                string titleColor = isUnlocked ? "#FFE86B" : "#8F7BB8";
                string descColor = isUnlocked ? "#EDE6FF" : "#786B96";
                sb.Append(icon).Append("  <color=").Append(titleColor).Append("><b>").Append(def.title).Append("</b></color>");
                sb.Append(isUnlocked ? "  <color=#00F1FF>DESBLOQUEADO</color>\n" : "  <color=#9E8DD8>BLOQUEADO</color>\n");
                sb.Append("<size=15><color=").Append(descColor).Append(">").Append(def.description).Append("</color></size>\n\n");
            }
            achievementsPanelBodyText.text = sb.ToString();
        }
        ...
        private void AnimateAchievementsButton()
        {
            ...
            {
                float alpha = achievementsButtonHover ? 0.36f : 0.16f + Mathf.Sin(pulse * 1.2f + 2.5f) * 0.05f;
                achievementsButtonGlow.color = new Color(1f, 0.78f, 0.16f, alpha);
            }
            if (achievementsButtonIconImage != null)
            {
                Color animated = Color.Lerp(new Color(1.0f, 0.82f, 0.20f, 1f), NeonPink, (Mathf.Sin(pulse * 0.8f + 2.4f) + 1f) * 0.5f);
