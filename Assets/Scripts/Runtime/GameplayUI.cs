...
namespace ProjectBeat.Runtime
{
    ...
    public class GameplayUI : MonoBehaviour
    {
        ...
        public void Initialize(BeatmapData beatmap)
        {
            ...
            SetGameplayKeyIndicatorsVisible(!IsAndroidRuntime);
        ...
        private void RepositionGameplayHud()
        {
            ...
            if (IsAndroidRuntime)
            {
                // Avance 98: HUD movil limpio. En Android no se muestra el boton/guia
                // de PAUSA de PC ni los indicadores duplicados D/F/J/K del HUD.
                // El puntaje y la precision quedan compactos para no tapar la pista.
                if (hudInfoPanel != null)
                {
                    hudInfoPanel.anchoredPosition = new Vector2(-18f, -18f);
                    hudInfoPanel.sizeDelta = new Vector2(315f, 150f);
                }

                SetTmpVisible(songText, false);
                SetTmpVisible(pauseHintHudText, false);
                PositionTmp(scoreText, new Vector2(1f, 1f), new Vector2(-34f, -46f), new Vector2(260f, 48f), TextAlignmentOptions.TopRight);
                PositionTmp(accuracyText, new Vector2(1f, 1f), new Vector2(-34f, -96f), new Vector2(260f, 44f), TextAlignmentOptions.TopRight);
                ConfigureHudText(scoreText, 21f, 0.6f);
                ConfigureHudText(accuracyText, 20f, 0.6f);
                return;
            }

            SetTmpVisible(songText, true);
            SetTmpVisible(pauseHintHudText, true);
        ...
        private void EnsureHudInfoPanel()
        {
            if (hudInfoPanel != null)
            {
                hudInfoPanel.anchoredPosition = IsAndroidRuntime ? new Vector2(-18f, -18f) : new Vector2(-24f, -26f);
                hudInfoPanel.sizeDelta = IsAndroidRuntime ? new Vector2(315f, 150f) : new Vector2(430f, 320f);
                return;
            }
        ...
        {
            SetGameplayKeyIndicatorsVisible(visible && !IsAndroidRuntime);
            SetTmpVisible(songText, visible && !IsAndroidRuntime);
            ...
            if (pauseHintHudText != null) SetTmpVisible(pauseHintHudText, visible && !IsAndroidRuntime);
        ...
        private void Update()
        {
            ...
            EnsureGameplayKeyIndicators();
            bool showPcKeyIndicators = !resultsVisible && !IsAndroidRuntime;
            SetGameplayKeyIndicatorsVisible(showPcKeyIndicators);
            if (showPcKeyIndicators)
