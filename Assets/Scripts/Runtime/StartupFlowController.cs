...
namespace ProjectBeat.Runtime
{
    ...
    public class StartupFlowController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (isMainMenuSettingsOpen)
            {
                HandleMainMenuSettingsInput();
                return;
            }
        ...
        private void CreateMainMenuSettingsButton(Transform parent)
        {
            ...
            settingsButtonIcon = CreateText("SettingsGearIcon", go.transform, "⚙", 54, NeonCyan, FontStyles.Bold);
            settingsButtonIcon.alignment = TextAlignmentOptions.Center;
            settingsButtonIcon.rectTransform.sizeDelta = new Vector2(96f, 78f);
            settingsButtonIcon.rectTransform.anchoredPosition = new Vector2(0f, 8f);
            ...
            EventTrigger.Entry enter = new EventTrigger.Entry { eventID = EventTriggerType.PointerEnter };
            enter.callback.AddListener((_) => { if (settingsButtonBg != null) settingsButtonBg.color = new Color(0.02f, 0.16f, 0.23f, 0.96f); });
            trigger.triggers.Add(enter);

            EventTrigger.Entry exit = new EventTrigger.Entry { eventID = EventTriggerType.PointerExit };
            exit.callback.AddListener((_) => { if (settingsButtonBg != null) settingsButtonBg.color = new Color(0.015f, 0.045f, 0.075f, 0.88f); });
        ...
        {
            if (mainMenuSettingsGroup != null) return;

            mainMenuSettingsGroup = CreateGroup("MainMenuSettings", canvas.transform);
            Image dim = CreateImage("SettingsDim", mainMenuSettingsGroup.transform, new Color(0f, 0f, 0f, 0.68f));
            Stretch(dim.rectTransform);

            Image card = CreateImage("SettingsCard", mainMenuSettingsGroup.transform, new Color(0.01f, 0.027f, 0.05f, 0.98f));
            card.sprite = MakeSprite(Color.white);
            card.type = Image.Type.Sliced;
            card.rectTransform.sizeDelta = new Vector2(900f, 620f);
            card.rectTransform.anchoredPosition = Vector2.zero;

            CreateLine("SettingsTopLine", mainMenuSettingsGroup.transform, new Vector2(0f, 286f), new Vector2(730f, 4f), NeonCyan);
            CreateLine("SettingsBottomLine", mainMenuSettingsGroup.transform, new Vector2(0f, -286f), new Vector2(520f, 3f), NeonOrange);

            mainMenuSettingsTitle = CreateText("SettingsTitle", mainMenuSettingsGroup.transform, "CONFIGURACION", 38, NeonYellow, FontStyles.Bold);
            mainMenuSettingsTitle.alignment = TextAlignmentOptions.Center;
            mainMenuSettingsTitle.characterSpacing = 5f;
            mainMenuSettingsTitle.rectTransform.sizeDelta = new Vector2(780f, 56f);
            mainMenuSettingsTitle.rectTransform.anchoredPosition = new Vector2(0f, 238f);

            mainMenuSettingsLabels = new TMP_Text[MainMenuSettingsOptionCount];
            mainMenuSettingsValues = new TMP_Text[MainMenuSettingsOptionCount];
            mainMenuSettingsRows = new Image[MainMenuSettingsOptionCount];
            ...
            string[] labels =
            {
                ...
                "RESOLUCION",
                "MODO PANTALLA",
                "INTENSIDAD EFECTOS VISUALES",
                "MODO SENSIBILIDAD VISUAL",
                "VOLUMEN GENERAL",
                "VOLVER"
            };
            string[] descriptions =
            {
                "Ajusta la iluminacion general.",
                "Cambia la resolucion de pantalla.",
                "Alterna pantalla completa, ventana o sin bordes.",
                "Regula glow, flashes, particulas y transiciones.",
                "Reduce destellos rapidos para mayor comodidad visual.",
                "Ajusta el volumen principal.",
                "Regresa al menu principal."
            };

            float startY = 160f;
            for (int i = 0; i < MainMenuSettingsOptionCount; i++)
            {
                int optionIndex = i;
                Image row = CreateImage("SettingsRow_" + i, mainMenuSettingsGroup.transform, new Color(0f, 0f, 0f, 0f));
                row.sprite = MakeSprite(Color.white);
                row.type = Image.Type.Sliced;
                row.rectTransform.sizeDelta = new Vector2(720f, 55f);
                row.rectTransform.anchoredPosition = new Vector2(0f, startY - i * 64f);
                mainMenuSettingsRows[i] = row;

                mainMenuSettingsLabels[i] = CreateText("SettingsLabel_" + i, row.transform, labels[i] + "\n<size=12><color=#AEB6D0>" + descriptions[i] + "</color></size>", 18, NeonCyan, FontStyles.Bold);
                mainMenuSettingsLabels[i].alignment = TextAlignmentOptions.Left;
                mainMenuSettingsLabels[i].rectTransform.sizeDelta = new Vector2(440f, 52f);
                mainMenuSettingsLabels[i].rectTransform.anchoredPosition = new Vector2(-126f, 0f);
                mainMenuSettingsLabels[i].raycastTarget = false;

                mainMenuSettingsValues[i] = CreateText("SettingsValue_" + i, row.transform, "", 18, TextNormal, FontStyles.Bold);
                mainMenuSettingsValues[i].alignment = TextAlignmentOptions.Right;
                mainMenuSettingsValues[i].rectTransform.sizeDelta = new Vector2(230f, 44f);
                mainMenuSettingsValues[i].rectTransform.anchoredPosition = new Vector2(230f, 0f);
                mainMenuSettingsValues[i].raycastTarget = false;

                EventTrigger trigger = row.gameObject.AddComponent<EventTrigger>();
                trigger.triggers = new System.Collections.Generic.List<EventTrigger.Entry>();
                EventTrigger.Entry enter = new EventTrigger.Entry { eventID = EventTriggerType.PointerEnter };
                enter.callback.AddListener((_) => { selectedMainMenuSettingsOption = optionIndex; RefreshMainMenuSettingsPanel(); });
                trigger.triggers.Add(enter);
                EventTrigger.Entry click = new EventTrigger.Entry { eventID = EventTriggerType.PointerClick };
                click.callback.AddListener((_) => ConfirmOrAdjustMainMenuSetting(optionIndex));
                trigger.triggers.Add(click);
            }

            mainMenuSettingsHint = CreateText("SettingsHint", mainMenuSettingsGroup.transform, "[W/S] Seleccionar   [A/D] Ajustar   [ENTER/MOUSE] Confirmar   [ESC] Volver", 14, TextDim, FontStyles.Bold);
            mainMenuSettingsHint.alignment = TextAlignmentOptions.Center;
            mainMenuSettingsHint.rectTransform.sizeDelta = new Vector2(820f, 32f);
            mainMenuSettingsHint.rectTransform.anchoredPosition = new Vector2(0f, -252f);

            ShowMainMenuSettings(false, true);
        }

        private void OpenMainMenuSettings()
        {
            if (menuGroup == null || mainMenuSettingsGroup == null) return;
            isMainMenuSettingsOpen = true;
            selectedMainMenuSettingsOption = 0;
            LoadMainMenuSettingsPrefs();
            menuGroup.interactable = false;
            menuGroup.blocksRaycasts = false;
            menuGroup.alpha = 0f;
            ShowMainMenuSettings(true, true);
            RefreshMainMenuSettingsPanel();
        }
        ...
        private void ShowMainMenuSettings(bool show, bool instant = false)
        {
            if (mainMenuSettingsGroup == null) return;
            mainMenuSettingsGroup.alpha = show ? 1f : 0f;
            mainMenuSettingsGroup.interactable = show;
            mainMenuSettingsGroup.blocksRaycasts = show;
        }

        private void HandleMainMenuSettingsInput()
        {
            ...
            if (Input.GetKeyDown(KeyCode.UpArrow) || Input.GetKeyDown(KeyCode.W))
            {
                selectedMainMenuSettingsOption = (selectedMainMenuSettingsOption - 1 + MainMenuSettingsOptionCount) % MainMenuSettingsOptionCount;
                RefreshMainMenuSettingsPanel();
            }
            else if (Input.GetKeyDown(KeyCode.DownArrow) || Input.GetKeyDown(KeyCode.S))
            {
                selectedMainMenuSettingsOption = (selectedMainMenuSettingsOption + 1) % MainMenuSettingsOptionCount;
                RefreshMainMenuSettingsPanel();
            }
            else if (Input.GetKeyDown(KeyCode.LeftArrow) || Input.GetKeyDown(KeyCode.A))
            {
                AdjustMainMenuSetting(selectedMainMenuSettingsOption, -1);
            }
            ...
            {
                ConfirmOrAdjustMainMenuSetting(selectedMainMenuSettingsOption);
            }
        }

        private void ConfirmOrAdjustMainMenuSetting(int option)
        {
            if (option == 4)
            {
            ...
            AdjustMainMenuSetting(option, 1);
        }

        private void AdjustMainMenuSetting(int option, int dir)
        {
            LoadMainMenuSettingsPrefs();
            switch (option)
            {
                case 0:
                    menuBrightness = Mathf.Clamp(menuBrightness + dir * 0.05f, 0.55f, 1.35f);
                    PlayerPrefs.SetFloat(BrightnessPrefsKey, menuBrightness);
                    break;
                case 1:
                    menuResolutionIndex = WrapIndex(menuResolutionIndex + dir, ResolutionOptions.Length);
                    PlayerPrefs.SetInt(ResolutionPrefsKey, menuResolutionIndex);
                    ApplyMainMenuDisplaySettings();
                    break;
                case 2:
                    menuDisplayModeIndex = WrapIndex(menuDisplayModeIndex + dir, DisplayModeNames.Length);
                    PlayerPrefs.SetInt(DisplayModePrefsKey, menuDisplayModeIndex);
                    ApplyMainMenuDisplaySettings();
                    break;
                case 3:
                    VisualAccessibilitySettings.AdjustIntensity(dir);
                    break;
                case 4:
                    VisualAccessibilitySettings.ToggleSensitivityMode();
                    break;
                case 5:
                    menuVolume = Mathf.Clamp01(menuVolume + dir * 0.05f);
                    PlayerPrefs.SetFloat(MasterVolumePrefsKey, menuVolume);
                    AudioListener.volume = menuVolume;
                    if (menuMusicSource != null) menuMusicSource.volume = 0.46f * menuVolume;
                    break;
            }
            PlayerPrefs.Save();
            RefreshMainMenuSettingsPanel();
        ...
        private void LoadMainMenuSettingsPrefs()
        {
            menuBrightness = Mathf.Clamp(PlayerPrefs.GetFloat(BrightnessPrefsKey, 1f), 0.55f, 1.35f);
            menuVolume = Mathf.Clamp01(PlayerPrefs.GetFloat(MasterVolumePrefsKey, 1f));
            menuResolutionIndex = Mathf.Clamp(PlayerPrefs.GetInt(ResolutionPrefsKey, 3), 0, ResolutionOptions.Length - 1);
            menuDisplayModeIndex = Mathf.Clamp(PlayerPrefs.GetInt(DisplayModePrefsKey, 0), 0, DisplayModeNames.Length - 1);
        }

        private void RefreshMainMenuSettingsPanel()
        {
            LoadMainMenuSettingsPrefs();
            if (mainMenuSettingsValues == null || mainMenuSettingsLabels == null) return;

            string[] values =
            {
                Mathf.RoundToInt(menuBrightness * 100f) + "%",
                ResolutionOptions[menuResolutionIndex].x + "x" + ResolutionOptions[menuResolutionIndex].y,
                DisplayModeNames[menuDisplayModeIndex],
                VisualAccessibilitySettings.IntensityName,
                VisualAccessibilitySettings.SensitivityMode ? "ON" : "OFF",
                Mathf.RoundToInt(menuVolume * 100f) + "%",
                "REGRESAR"
            };

            for (int i = 0; i < MainMenuSettingsOptionCount; i++)
            {
                bool selected = i == selectedMainMenuSettingsOption;
                if (mainMenuSettingsValues[i] != null)
                {
                    mainMenuSettingsValues[i].text = selected && i < 6 ? "<color=#FF6A00><</color> " + values[i] + " <color=#FF6A00>></color>" : values[i];
                    mainMenuSettingsValues[i].color = selected ? NeonYellow : TextNormal;
                }
                if (mainMenuSettingsLabels[i] != null)
                    mainMenuSettingsLabels[i].color = selected ? NeonYellow : NeonCyan;
                if (mainMenuSettingsRows[i] != null)
                    mainMenuSettingsRows[i].color = selected ? new Color(1f, 0.42f, 0.02f, 0.18f) : new Color(0.015f, 0.04f, 0.07f, 0.18f);
            }
        }

        private void ApplyMainMenuDisplaySettings()
        {
            Vector2Int r = ResolutionOptions[Mathf.Clamp(menuResolutionIndex, 0, ResolutionOptions.Length - 1)];
            FullScreenMode mode = FullScreenMode.FullScreenWindow;
            if (menuDisplayModeIndex == 1) mode = FullScreenMode.Windowed;
            else if (menuDisplayModeIndex == 2) mode = FullScreenMode.FullScreenWindow;
        ...
        private void AnimateSettingsButton()
        {
            ...
            if (settingsButtonIcon != null)
                settingsButtonIcon.color = Color.Lerp(NeonCyan, NeonOrange, (Mathf.Sin(pulse * 0.75f) + 1f) * 0.5f);
        }
