...
namespace ProjectBeat.Runtime
{
    public class GameController : MonoBehaviour
    {
        ...
        [Header("Basic Timing Calibration")]
        [SerializeField] private float offsetStep = 0.005f;
        private float currentTimingOffset;
        private GUIStyle offsetStyle;

        private static readonly int[] MilestoneCombos = { 10, 25, 50 };
        ...
        private void Start()
        {
            ...
            currentTimingOffset = PlayerPrefs.GetFloat("ProjectBeat_TimingOffset", 0f);
            if (conductor != null)
                conductor.SetCalibrationOffset(currentTimingOffset);
        ...
        private void Update()
        {
            HandleTimingOffsetInput();

            if (Input.GetKeyDown(KeyCode.R) && (finished || !IsGameplayRunning))
                RestartScene();
        }

        private void HandleTimingOffsetInput()
        {
            ...
            {
                currentTimingOffset -= offsetStep;
                changed = true;
            }
            ...
            {
                currentTimingOffset = 0f;
                changed = true;
            }

            if (changed)
            {
                currentTimingOffset = Mathf.Clamp(currentTimingOffset, -0.30f, 0.30f);
                conductor.SetCalibrationOffset(currentTimingOffset);
            }
        }
        ...
        private void OnGUI()
        {
            if (!IsGameplayRunning || finished) return;

            if (offsetStyle == null)
            {
                offsetStyle = new GUIStyle(GUI.skin.label);
                offsetStyle.fontSize = 18;
                offsetStyle.normal.textColor = Color.white;
                offsetStyle.alignment = TextAnchor.UpperRight;
            }

            string sign = currentTimingOffset >= 0f ? "+" : "";
            string text = $"OFFSET {sign}{currentTimingOffset * 1000f:0} ms\nF2 -  F3 +  F4 reset";
            GUI.Label(new Rect(Screen.width - 260, 18, 240, 60), text, offsetStyle);
        }
