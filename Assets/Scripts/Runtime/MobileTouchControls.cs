...
{
    /// <summary>
    /// Avance 99 – version definitiva final del juego basada en la optimizacion Android.
    ///
    ...
    /// para D/F/J/K con botones mas limpios, zonas tactiles mas comodas,
    /// feedback visual al presionar y sin boton de PAUSA superior.
    /// </summary>
    ...
    public sealed class MobileTouchControls : MonoBehaviour
    {
        ...
        private Image[] laneButtonImages;
        ...
        private void BuildHud()
        {
            ...
            // Avance 98: controles mas comodos en telefono real. Se separan de la zona
            // central de notas y se deja el centro limpio para ver el gameplay.
            laneButtonRects[0] = CreateLaneButton(controls.transform, "PB_Touch_D", "D", 0, new Vector2(168f, 122f), TextAnchor.LowerLeft);
            laneButtonRects[1] = CreateLaneButton(controls.transform, "PB_Touch_F", "F", 1, new Vector2(318f, 122f), TextAnchor.LowerLeft);
            laneButtonRects[2] = CreateLaneButton(controls.transform, "PB_Touch_J", "J", 2, new Vector2(-318f, 122f), TextAnchor.LowerRight);
            laneButtonRects[3] = CreateLaneButton(controls.transform, "PB_Touch_K", "K", 3, new Vector2(-168f, 122f), TextAnchor.LowerRight);
        ...
        private RectTransform CreateLaneButton(Transform parent, string name, string label, int laneIndex, Vector2 anchoredPosition, TextAnchor anchor)
        {
            ...
            image.color = new Color(0.55f, 0.42f, 1f, 0.78f);
            ...
            TMP_Text text = CreateText(go.transform, "Label", label, 50f, FontStyles.Bold);
            text.color = new Color(1f, 0.96f, 1f, 0.96f);
            text.alignment = TextAlignmentOptions.Center;

            if (laneButtonImages != null && laneIndex >= 0 && laneIndex < laneButtonImages.Length)
                laneButtonImages[laneIndex] = image;
        ...
        private void CreateHintLabel(Transform parent)
        {
            ...
            text.text = "Android touch";
            text.fontSize = 13f;
            text.alignment = TextAlignmentOptions.Center;
            text.color = new Color(0.95f, 0.95f, 1f, 0.28f);
        ...
        private void PollAndroidTouches()
        {
            ...
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                // Avance 98: zona tactil ampliada solo en la franja baja de Android.
                // Ayuda cuando el dedo toca el borde del boton sin invadir el centro de gameplay.
                if (!matchedButton)
                {
                    int expandedLane = ResolveExpandedMobileLane(pos);
                    if (expandedLane >= 0 && expandedLane < 4)
                        touching[expandedLane] = true;
                }
            }
        ...
        private int ResolveExpandedMobileLane(Vector2 screenPoint)
        {
            if (Screen.width <= 0 || Screen.height <= 0)
                return -1;

            float yLimit = Screen.height * 0.30f;
            if (screenPoint.y > yLimit)
                return -1;

            float x = screenPoint.x / Screen.width;

            // Mantiene un espacio muerto en el centro para no robar toques accidentales
            // sobre la pista, y solo expande los bordes alrededor de D/F/J/K.
            if (x < 0.15f) return 0;
            if (x < 0.32f) return 1;
            if (x > 0.85f) return 3;
            if (x > 0.68f) return 2;
        ...
        private void UpdateLaneButtonVisuals()
        {
            if (laneButtonRects == null || laneButtonImages == null)
                return;

            for (int i = 0; i < 4; i++)
            {
                RectTransform rt = i < laneButtonRects.Length ? laneButtonRects[i] : null;
                Image image = i < laneButtonImages.Length ? laneButtonImages[i] : null;
                bool pressed = i < lanePressed.Length && lanePressed[i];

                if (rt != null)
                {
                    float targetScale = pressed ? 1.10f : 1.0f;
                    rt.localScale = Vector3.Lerp(rt.localScale, Vector3.one * targetScale, Time.unscaledDeltaTime * 18f);
                }

                if (image != null)
                {
                    Color normal = new Color(0.55f, 0.42f, 1f, 0.78f);
                    Color active = new Color(0.00f, 0.92f, 1f, 0.93f);
                    image.color = Color.Lerp(image.color, pressed ? active : normal, Time.unscaledDeltaTime * 16f);
                }
            }
        }
