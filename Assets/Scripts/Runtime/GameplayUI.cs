...
namespace ProjectBeat.Runtime
{
    ...
    public class GameplayUI : MonoBehaviour
    {
        ...
        private const float ResultAnimDuration = 0.55f;
        ...
        public void Initialize(BeatmapData beatmap)
        {
            ...
            if (resultGroup != null)
            {
                ...
                resultGroup.transform.localScale = new Vector3(0.94f, 0.94f, 1f);
            }
        ...
        public void ShowResults(ScoreManager sm, BeatmapData beatmap)
        {
            ...
            resultGroup.transform.localScale = new Vector3(0.94f, 0.94f, 1f);
            resultAnimTimer = ResultAnimDuration;
            ...
            string song = beatmap != null ? beatmap.songName : "NIVEL";

            if (resultTitleText != null)
                resultTitleText.text =
                    $"<size=22><color=#66E8FF>RESULTADOS</color></size>\n" +
                    $"<b><color=#FFFFFF>{song}</color></b>\n" +
                    $"<size=86><color=#{rh}>{rank}</color></size>";

            if (resultBodyText != null)
                resultBodyText.text =
                    $"<color=#FFDD55>PUNTAJE TOTAL</color>     <b>{sm.Score:N0}</b>\n" +
                    $"<color=#FFAA44>PRECISION</color>         <b>{sm.Accuracy:0.00}%</b>\n" +
                    $"<color=#AAFFAA>MAX COMBO</color>         <b>{sm.MaxCombo}</b>\n\n" +
                    $"<color=#FFFF88>PERFECT</color>  <b>{sm.PerfectCount}</b>     " +
                    $"<color=#88FF88>GOOD</color>  <b>{sm.GoodCount}</b>\n" +
                    $"<color=#FFAA44>BAD</color>      <b>{sm.BadCount}</b>     " +
                    $"<color=#FF5555>MISS</color>  <b>{sm.MissCount}</b>\n\n" +
                    $"<size=22><color=#BBBBBB>[ R ] Reiniciar   [ Q ] Volver al selector   [ ESC ] Pausa</color></size>";
        }
        ...
        private void Update()
        {
            ...
            // Results smooth appearance
            if (resultGroup != null && resultAnimTimer > 0f)
            {
                resultAnimTimer -= Time.deltaTime;
                float t = 1f - Mathf.Clamp01(resultAnimTimer / ResultAnimDuration);
                float eased = 1f - Mathf.Pow(1f - t, 3f);
                resultGroup.alpha = eased;
