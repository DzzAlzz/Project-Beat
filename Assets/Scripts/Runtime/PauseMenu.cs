...
namespace ProjectBeat.Runtime
{
    ...
    public class PauseMenu : MonoBehaviour
    {
        ...
        private void RefreshSettingsLabels()
        {
            ...
            if (settingsGraphicsText != null)
            {
                settingsGraphicsText.text = settingsOption == 1
                    ? "<color=#FFF000>> GRAFICOS</color>    <color=#00F1FF>Brillo</color>  " + Mathf.RoundToInt(brightnessValue * 100f) + "%"
                    : "  GRAFICOS    <color=#00F1FF>Brillo</color>  " + Mathf.RoundToInt(brightnessValue * 100f) + "%";
            }

            if (settingsSoundText != null)
            {
                settingsSoundText.text = settingsOption == 2
                    ? "<color=#FFF000>> SONIDO</color>      <color=#00F1FF>Volumen general</color>  " + Mathf.RoundToInt(volumeValue * 100f) + "%"
                    : "  SONIDO      <color=#00F1FF>Volumen general</color>  " + Mathf.RoundToInt(volumeValue * 100f) + "%";
            }

            if (settingsBackText != null)
                settingsBackText.text = settingsOption == 3 ? "<color=#FFF000>> VOLVER</color>" : "  VOLVER";

            UpdateSliderVisual(brightnessSliderTrack, brightnessSliderFill, brightnessSliderHandle, brightnessSliderGlow, brightnessValue, settingsOption == 1);
            UpdateSliderVisual(volumeSliderTrack, volumeSliderFill, volumeSliderHandle, volumeSliderGlow, volumeValue, settingsOption == 2);
        ...
        private void UpdateSliderVisual(RectTransform track, Image fill, Image handle, Image glow, float value, bool selected)
        {
            ...
            if (fill != null)
            {
                ...
                fillRt.anchorMax = new Vector2(value, 0.5f);
                fillRt.offsetMin = new Vector2(0f, -5f);
                fillRt.offsetMax = new Vector2(0f, 5f);
            ...
            if (handle != null && track != null)
            {
                ...
                float width = track.rect.width <= 1f ? 320f : track.rect.width;
                handleRt.anchoredPosition = new Vector2(Mathf.Lerp(-width * 0.5f, width * 0.5f, value), 0f);
                handleRt.localScale = selected ? new Vector3(1.18f, 1.18f, 1f) : Vector3.one;
                handle.color = selected ? NeonYellow : TextNormal;
            }

            if (glow != null)
                glow.color = selected ? new Color(1f, 0.88f, 0.05f, 0.22f) : new Color(0f, 0.8f, 1f, 0.08f);
        }
        ...
        {
            TMP_Text[] allTexts = { resumeLabel, selectLevelLabel, restartLabel, quitLabel, levelNameText, levelArtistText, levelHintText, settingsTitleText, settingsBodyText, settingsGraphicsText, settingsSoundText, settingsBackText, settingsHintText };
        ...
        private void BuildBasicSettingsPanel()
        {
            ...
            CreateCard(root.transform, "PB_Settings_Card", new Vector2(820f, 560f), Vector2.zero, 2);
            CreateLine(root.transform, "PB_Settings_TopLine", new Vector2(0f, 250f), new Vector2(650f, 4f), NeonCyan, 3);
            settingsTitleText = CreateTmp(root.transform, "PB_Settings_Title", "CONFIGURACION", new Vector2(0f, 200f), new Vector2(660f, 54f), 42f, NeonYellow, 4, FontStyles.Bold, 5f);
            settingsBodyText = CreateTmp(root.transform, "PB_Settings_Body", "", new Vector2(-10f, 80f), new Vector2(690f, 170f), 18f, TextNormal, 5, FontStyles.Normal, 1.2f);
            settingsBodyText.alignment = TextAlignmentOptions.Left;

            settingsGraphicsText = CreateTmp(root.transform, "PB_Settings_GraphicsLabel", "", new Vector2(-10f, -34f), new Vector2(690f, 34f), 18f, TextNormal, 6, FontStyles.Normal, 1.2f);
            settingsGraphicsText.alignment = TextAlignmentOptions.Left;
            brightnessSliderTrack = CreateSlider(root.transform, "PB_Settings_BrightnessSlider", new Vector2(110f, -74f), new Vector2(420f, 20f), out brightnessSliderFill, out brightnessSliderHandle, out brightnessSliderGlow);

            settingsSoundText = CreateTmp(root.transform, "PB_Settings_SoundLabel", "", new Vector2(-10f, -126f), new Vector2(690f, 34f), 18f, TextNormal, 10, FontStyles.Normal, 1.2f);
            settingsSoundText.alignment = TextAlignmentOptions.Left;
            volumeSliderTrack = CreateSlider(root.transform, "PB_Settings_VolumeSlider", new Vector2(110f, -166f), new Vector2(420f, 20f), out volumeSliderFill, out volumeSliderHandle, out volumeSliderGlow);

            settingsBackText = CreateTmp(root.transform, "PB_Settings_BackLabel", "", new Vector2(-10f, -214f), new Vector2(690f, 34f), 18f, TextNormal, 14, FontStyles.Normal, 1.2f);
            settingsBackText.alignment = TextAlignmentOptions.Left;
            settingsHintText = CreateTmp(root.transform, "PB_Settings_Hint", "", new Vector2(0f, -258f), new Vector2(720f, 30f), 16f, TextNormal, 15, FontStyles.Normal, 1.5f);
            ApplyBrightness();
            RefreshSettingsLabels();
        }
        ...
        private RectTransform CreateSlider(Transform parent, string name, Vector2 pos, Vector2 size, out Image fill, out Image handle, out Image glow)
        {
            GameObject trackGo = new GameObject(name);
            trackGo.transform.SetParent(parent, false);
            Image track = trackGo.AddComponent<Image>();
            track.raycastTarget = false;
            track.sprite = lineSprite;
            track.type = Image.Type.Sliced;
            track.color = new Color(0.08f, 0.13f, 0.18f, 0.92f);
            Outline outline = trackGo.AddComponent<Outline>();
            outline.effectColor = new Color(0f, 0.95f, 1f, 0.25f);
            outline.effectDistance = new Vector2(1.2f, -1.2f);

            RectTransform rt = trackGo.GetComponent<RectTransform>();
            rt.anchorMin = new Vector2(0.5f, 0.5f);
            rt.anchorMax = new Vector2(0.5f, 0.5f);
            rt.pivot = new Vector2(0.5f, 0.5f);
            ...
            GameObject glowGo = new GameObject(name + "_Glow");
            glowGo.transform.SetParent(trackGo.transform, false);
            glow = glowGo.AddComponent<Image>();
            glow.raycastTarget = false;
            glow.sprite = lineSprite;
            glow.type = Image.Type.Sliced;
            glow.color = new Color(0f, 0.8f, 1f, 0.08f);
            RectTransform glowRt = glowGo.GetComponent<RectTransform>();
            glowRt.anchorMin = Vector2.zero;
            glowRt.anchorMax = Vector2.one;
            glowRt.offsetMin = new Vector2(-8f, -7f);
            glowRt.offsetMax = new Vector2(8f, 7f);
            ...
            handle = handleGo.AddComponent<Image>();
            handle.raycastTarget = false;
            handle.sprite = roundedButtonSprite;
            ...
            handleRt.sizeDelta = new Vector2(24f, 28f);
