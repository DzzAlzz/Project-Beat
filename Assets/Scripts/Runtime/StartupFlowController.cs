...
namespace ProjectBeat.Runtime
{
    ...
    public class StartupFlowController : MonoBehaviour
    {
        ...
        private void BuildUI()
        {
            ...
            menuTitleGlow = CreateImage("MenuTitleNeonGlow", menuGroup.transform, new Color(1f, 0.92f, 0.02f, 0.12f));
            menuTitleGlow.sprite = MakeRingSprite(128, 0.48f, 0.32f);
            menuTitleGlow.rectTransform.sizeDelta = new Vector2(640f, 150f);
            ...
            CreateLine("MainPanelTopAccent", menuGroup.transform, new Vector2(0f, 238f), new Vector2(540f, 3f), NeonCyan);
            CreateLine("MainPanelBottomAccent", menuGroup.transform, new Vector2(0f, -238f), new Vector2(540f, 3f), NeonOrange);
        ...
        private void CreateMainMenuMusicBackground(Transform parent)
        {
            ...
            {
                float x = -510f + i * 60f;
                Image bar = CreateLine("MenuEqualizerBar_" + i, parent, new Vector2(x, -292f), new Vector2(18f, 34f), i % 3 == 0 ? NeonOrange : NeonCyan);
                bar.color = new Color(bar.color.r, bar.color.g, bar.color.b, 0.13f);
        ...
        private void CreateMenuBeatRings(Transform parent)
        {
            ...
            {
                Color c = i % 2 == 0 ? NeonCyan : NeonOrange;
                Image ring = CreateImage("MainPanelBeatRing_" + i, parent, new Color(c.r, c.g, c.b, 0.10f));
        ...
        private void AnimateMusicalBackground(float wave)
        {
            if (equalizerBars != null)
            {
                for (int i = 0; i < equalizerBars.Length; i++)
                {
                    ...
                    Color c = i % 3 == 0 ? NeonOrange : NeonCyan;
        ...
        private void AnimateSettingsButton()
        {
            ...
            {
                Color normal = new Color(0.015f, 0.045f, 0.075f, 0.88f);
                Color hover = new Color(0.06f, 0.16f, 0.23f, 0.98f);
            ...
            if (settingsButtonGlow != null)
            {
                ...
                settingsButtonGlow.color = new Color(0f, 0.92f, 1f, glow);
            }

            if (settingsButtonIconImage != null)
            {
                Color animated = Color.Lerp(NeonCyan, NeonOrange, (Mathf.Sin(pulse * 0.75f) + 1f) * 0.5f);
                settingsButtonIconImage.color = Color.Lerp(settingsButtonIconImage.color, settingsButtonHover ? NeonOrange : animated, Time.unscaledDeltaTime * 10f);
            }
        }
        ...
        private RectTransform CreateModeButton(Transform parent, string name, Vector2 pos, bool available, out Image bg, out TMP_Text label, string textOverride = null, string subOverride = null)
        {
            ...
            Color accentColor = available ? NeonCyan : NeonOrange;
            ...
            Image bottomAccent = CreateLine("ButtonBottomNeonEdge", go.transform, new Vector2(0f, -37f), new Vector2(500f, 2.4f), NeonOrange);
            bottomAccent.color = new Color(NeonOrange.r, NeonOrange.g, NeonOrange.b, available ? 0.22f : 0.16f);
        ...
        private void AnimateMenuVisuals()
        {
            ...
            if (menuTitleGlow != null)
            {
                ...
                menuTitleGlow.color = new Color(1f, 0.92f, 0.02f, glowAlpha);
        ...
        private void AnimateButton(RectTransform rt, Image bg, Image glow, TMP_Text label, bool selected, bool available)
        {
            ...
            Color highlight = available ? new Color(1f, 0.42f, 0.02f, 0.90f) : new Color(0.25f, 0.18f, 0.30f, 0.76f);
            bg.color = Color.Lerp(bg.color, selected ? highlight : normal, Time.unscaledDeltaTime * 14f);

            Color targetText = selected ? (available ? Color.white : NeonOrange) : (available ? NeonYellow : TextDim);
            label.color = Color.Lerp(label.color, targetText, Time.unscaledDeltaTime * 14f);

            if (glow != null)
            {
                Color glowColor = available ? NeonCyan : NeonOrange;
