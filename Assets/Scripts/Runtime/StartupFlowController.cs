...
namespace ProjectBeat.Runtime
{
    ...
    public class StartupFlowController : MonoBehaviour
    {
        ...
        private void CreateAchievementRow(ProfileAchievementStorage.AchievementDefinition def, bool unlocked, int index, float rowHeight, float spacing)
        {
            ...
            icon.sprite = MakeAchievementIconSprite(def.id, 96);
            icon.rectTransform.sizeDelta = new Vector2(48f, 48f);
            icon.rectTransform.anchoredPosition = new Vector2(-246f, 0f);
            icon.raycastTarget = false;

            TMP_Text title = CreateText("Title", row.transform, def.title, 17, unlocked ? Color.white : new Color(0.62f, 0.52f, 0.80f, 1f), FontStyles.Bold);
            ...
            TMP_Text desc = CreateText("Description", row.transform, def.description, 13, unlocked ? new Color(0.86f, 0.82f, 1f, 1f) : new Color(0.55f, 0.48f, 0.70f, 1f), FontStyles.Bold);
        ...
        private Sprite MakeAchievementIconSprite(string id, int size)
        {
            ...
            {
                case "FIRST_GAME": return MakeProfileIconSprite(size);
                case "FIRST_BEAT": return MakeMusicNoteIconSprite(size);
                case "COMBO_50": return MakeChainIconSprite(size);
                case "ACC_90": return MakeTargetIconSprite(size);
                case "RANK_A": return MakeRankLetterIconSprite(size, 'A');
                case "RANK_S": return MakeRankLetterIconSprite(size, 'S');
                case "FULL_COMBO": return MakeComboSparkIconSprite(size);
                case "MASTER_RHYTHM": return MakeCrownTrophyIconSprite(size);
                default: return MakeTrophySprite(size);
            }
        }
