...
namespace ProjectBeat.Runtime
{
    ...
    public class StartupFlowController : MonoBehaviour
    {
        ...
        private void BuildStatsPanel(Transform parent)
        {
            ...
            statsPanelBodyText = CreateText("StatsBody", bodyBox.transform, "", 16, TextNormal, FontStyles.Bold);
            ...
            statsPanelBodyText.rectTransform.sizeDelta = new Vector2(670f, 350f);
            statsPanelBodyText.rectTransform.anchoredPosition = new Vector2(0f, 0f);
        ...
        private void RefreshStatsPanel()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                sb.Append("  <color=#FF5A66>FALLO</color> ").Append(st.totalMiss);
                sb.Append("  <color=#9E8DD8>Ultimo:</color> ").Append(st.lastScore.ToString("0000000"));
                sb.Append(" / ").Append(st.lastAccuracy.ToString("0.00")).Append("% / ").Append(st.lastRank).Append("</size>\n\n");
            }
            if (data.levels.Count > count)
                sb.Append("<align=\"center\"><color=#9E8DD8>Hay mas niveles guardados. Resumen reducido para mantener la interfaz limpia.</color></align>");
